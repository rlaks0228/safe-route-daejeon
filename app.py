...
# ----------------------------------------------------
# 1. CCTV 없는 사고 지점 찾기 (sjoin_nearest 사용, 더 안정적인 방식)
# ----------------------------------------------------
def find_accidents_without_cctv(acc_gdf: gpd.GeoDataFrame,
                                cctv_gdf: gpd.GeoDataFrame,
                                radius_m: float = 50) -> gpd.GeoDataFrame:
    """
    ...
    # dist가 NaN이면 주변 radius_m 안에 CCTV가 없다는 뜻
...
def load_accident_layers(radius_m: float = 50) -> gpd.GeoDataFrame:
    ...
    st.write(f"[INFO] 사고 지점 전체: {len(acc_gdf)}")
    st.write(f"[INFO] CCTV 없는 사고 지점: {len(acc_no_cctv)}")
    st.write(f"[INFO] 빨간 점(합집합) 총 개수: {len(danger_points)}")
