...
def load_graph_and_scores():
    ...
    # 5) 지표 기준 (어두운 구간 / CCTV 취약 / 보호구역 인근) 임계값 계산
    lamp_vals_arr = np.array(lamp_vals)
    cctv_vals_arr = np.array(cctv_vals)
    child_vals_arr = np.array(child_vals)

    # lamp / cctv는 "낮을수록 취약", child는 "높을수록 안전"
    # 하위 20%, 상위 20% 기준 사용
    lamp_dark_thresh = float(np.quantile(lamp_vals_arr, 0.2)) if len(lamp_vals_arr) > 0 else 0.0
    cctv_low_thresh = float(np.quantile(cctv_vals_arr, 0.2)) if len(cctv_vals_arr) > 0 else 0.0
    child_high_thresh = float(np.quantile(child_vals_arr, 0.8)) if len(child_vals_arr) > 0 else 1.0
